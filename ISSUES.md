...
- [ ] **`title` "falls back to the first heading text" never happens.** Schema (`output-schema.ts:539`) says title *"Falls back to the first heading text only when no anchor has text."* But `navigationAnchorsors` filters out empty-text anchors (`list-detector.ts:91`), and `extractItem` returns `null` when `title` is empty (`:181-183`) — there is no heading fallback. **Fix:** drop the fallback clause from the description.
- [ ] **`url` "Empty when the anchor has no href" never happens.** Schema (`output-schema.ts:544`). Anchors without a navigation-worthy href are filtered (`isNavigationHref`, `:64-74`), and `extractItem` returns `null` when `url` is falsy (`:186-188`). Emitted `url` is always non-empty. **Fix:** drop the "Empty when…" clause.
