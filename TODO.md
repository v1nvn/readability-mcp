...
### STR-1 — `extract_url` with `render:"js"`  · Stretch · L  (deferred — not needed now)
- [x] Decide whether needed
- **Decision: Deferred.** chrome-devtools owns the browser and is the intended renderer; the server is deliberately fetch-free (zero outbound requests). The opt-in Puppeteer/Playwright peer-dep path stays available but unpulled until a concrete no-chrome-devtools audience asks for it. Re-open when real demand surfaces.
